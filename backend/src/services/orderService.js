...
function normalizeUf(value) {
  return requiredText(value, "UF do CRM");
}
...
async function validateSngpcDispensation(
  ...
  const digitalSignatureCode = String(
  ...
  let issueDateSource = payload.issueDate || "";
  let issueDate = issueDateSource ? new Date(issueDateSource) : null;
  if ((!issueDate || Number.isNaN(issueDate.getTime())) && targetItem.id_receita) {
    ...
    issueDateSource = ocrEmissao || issueDateSource;
  ...
  const rawIssueDate = String(issueDateSource || "").trim();
  const issueYear =
    Number(rawIssueDate.match(/^(\d{4})/)?.[1]) ||
    Number(rawIssueDate.match(/(\d{2})\/(\d{2})\/(\d{4})/)?.[3]) ||
    issueDate.getUTCFullYear();
  if (issueYear !== 2026) {
    const emissaoBr = `${String(issueDate.getUTCDate()).padStart(2, "0")}/${String(issueDate.getUTCMonth() + 1).padStart(2, "0")}/${issueDate.getUTCFullYear()}`;
    throw createError(
      `Receita vencida: a data de emissão (${emissaoBr}) não pertence a 2026. Não é possível registrar a dispensação no SNGPC.`,
      400,
    );
  }
